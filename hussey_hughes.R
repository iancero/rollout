run_hussey_hughes <- function(Va = .005, Vb = .05, Nreps = (.8 * .2 ) / .01^2) {
  ...
  # Va <- 0.005 # get from function input
  # Vb <- 0.05 # get from function input
  Ve <- 1 - Va - Vb # error variance


  #  get power near .8 to +/- .02 as CI
  # Nreps <-   ( (.8 * .2 ) / .01^2  ) # 1600 # get from function input
  ...
  for ( r in 1 : Nreps ) {
    ...
    m0.par.null <- suppressMessages ( lmerTest::lmer ( Y.null ~ -1 + TimeValue +  Tx + ( 1 | the.rows ) ))
    Stats [ r , 1:2 ] <- GrabTP ( m0.par.null , "Tx")


    m1.par.alt <- suppressMessages ( lmerTest::lmer ( Y ~ -1 + TimeValue + Tx + ( 1 | the.rows ) ))
    Stats [ r , 3:4 ] <- GrabTP ( m1.par.alt , "Tx" )

    ##  Stepped Wedge design

    m0.sw.null <- suppressMessages ( lmerTest::lmer ( Z.null ~ -1 + TimeValue + Cross + ( 1 | the.rows ) ))
    Stats [ r , 5:6 ] <- GrabTP ( m0.sw.null , "Cross")

    m1.sw.alt <- suppressMessages ( lmerTest::lmer ( Z ~ -1 + TimeValue + Cross + ( 1 | the.rows ) ) )
